...
const { fetchKpiMaps, fetchProwessTimeSeries, resolveMetric } = require('../utils/formulaRegistry');
...
async function getQ4Stats(ticker, companyName) {
  const [{ current: kpiMap, prev: prevKpiMap }, revSeries] = await Promise.all([
    fetchKpiMaps(prisma, companyName),
    fetchProwessTimeSeries(prisma, companyName, 'REV_OP'),
  ]);
  ...
  function rk(field, abbr) {
    const res = resolveMetric(abbr, { kpiMap, prevKpiMap });
...
module.exports = { getPeerDataForCall };
