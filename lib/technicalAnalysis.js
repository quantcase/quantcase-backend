...
class TechnicalAnalysis {
  ...
  async fetchMarketData(ticker) {
    const now = Date.now();
    const [daily, weekly, monthly, quoteResult, calendarResult, allTimeMonthly] = await Promise.allSettled([
      ...
      this.yf.chart(ticker, { period1: '1990-01-01', interval: '1mo' }),
    ]);
    ...
    // All-time high / low from max-history monthly bars
    const allTimeBars = this._normaliseBars(allTimeMonthly.status === 'fulfilled' ? allTimeMonthly.value?.quotes : []);
    ...
    return { dailyBars, weeklyBars, monthlyBars, quote, nextEarningsDate, allTimeHigh, allTimeLow };
  }
