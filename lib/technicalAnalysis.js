...
function aggregateBars(rows, interval) {
  ...
  for (const r of rows) {
    ...
    if (!isFinite(closeVal)) continue; // skip rows with null/NaN close
...
class TechnicalAnalysis {
  ...
  async fetchMarketData(symbol) {
    ...
    const [allDailyRows, athAtlRows] = await Promise.all([
      ...
      // DB-side ATH/ATL — fetch only the two extreme rows instead of all history
      prisma.$queryRaw`
        SELECT
          MAX(high) AS ath,
          MIN(low)  AS atl,
          (SELECT datetime FROM nse_equity WHERE symbol = ${symbol} AND high = (SELECT MAX(high) FROM nse_equity WHERE symbol = ${symbol}) ORDER BY datetime DESC LIMIT 1) AS ath_date,
          (SELECT datetime FROM nse_equity WHERE symbol = ${symbol} AND low  = (SELECT MIN(low)  FROM nse_equity WHERE symbol = ${symbol}) ORDER BY datetime DESC LIMIT 1) AS atl_date
        FROM nse_equity
        WHERE symbol = ${symbol}
      `,
    ]);
    ...
    let high52wDate = null;
    let low52wDate  = null;
    if (dailyBars.length > 0) {
      const h52Bar = dailyBars.reduce((a, b) => b.high > a.high ? b : a);
      const l52Bar = dailyBars.reduce((a, b) => b.low  < a.low  ? b : a);
      high52wDate = h52Bar.date;
      low52wDate  = l52Bar.date;
    }
