...
class Financials {
  ...
  async analyze(symbol) {
    ...
    const [
      ...
      this._buildTableFromConfig('financials.pnl.quarterly', resCtx, 'quarterly'),
      this._buildTableFromConfig('financials.pnl.annual', resCtx, 'annual'),
      this._buildTableFromConfig('financials.balance-sheet.annual', resCtx, 'annual'),
      this._buildTableFromConfig('financials.balance-sheet.quarterly', resCtx, 'quarterly'),
      this._buildTableFromConfig('financials.cashflow.annual', resCtx, 'annual'),
      this._buildTableFromConfig('financials.cashflow.quarterly', resCtx, 'quarterly'),
  ...
  // ── Generic ScreenConfig-driven table builder (P&L / Balance Sheet / Cash
  // Flow all share this) — replaces the old hardcoded row arrays. Every
  // value comes from resolveFormulaSeries, the same per-index engine
  // _seriesForAggregate already uses internally, walking the same
  // resCtx.getSeriesMap() every other registry consumer reads from — so a
  // row here can never silently diverge from what admin's preview endpoint
  // (which calls resolveMetric on the same resCtx shape) would show for the
  // current period.
  async _buildTableFromConfig(configKey, resCtx, freq) {
    const config = await prisma.screenConfig.findUnique({
      where:   { key: configKey },
      include: { items: { orderBy: { display_order: 'asc' } } },
    });
    if (!config || !config.items.length) return { periods: [], rows: [] };
    ...
    for (const item of config.items) {
      ...
      const rawValues = await resolveFormulaSeries(item.kpi_abbr, resCtx, { frequency: freq });
      const values    = rawValues.slice(sliceFrom).map((v) => roundTo(v, decimals));

      const row = { key: item.kpi_abbr, label: item.label ?? def?.name ?? item.kpi_abbr, values };
      if (item.highlight) row.highlight = true;
      if (def?.denomination === 'percentage') row.format = 'percent';
      if (item.expandable) row.meta = { expandable: true };
      rows.push(row);
    }
