...
const { fetchMarketSnapshot } = require('../utils/formulaRegistry/dataFetcherMarket');
...
class Financials {
  ...
  async analyze(symbol) {
    ...
    // below (3.3 of the screener-config rework), _buildMetrics' growth/ROE/
    // stock-CAGR figures, and the ROE/PB_TTM/valuation swaps in
    // _buildValuation — the exact same resolveMetric path GET
    // /admin/kpis/:abbr/preview uses, so these numbers are guaranteed to
    // match what admin sees there.
    //
    // byAnnualAbbr/byQtrAbbr (via the older ALL_ABBRS-scoped fetch) still
    // separately back _buildTTM's figures, unchanged by this pass — a
    // deliberate, minor duplicate-query tradeoff (two overlapping bulk
    // fetches instead of unifying onto resCtx everywhere) rather than
    // restructuring code the plan didn't ask to touch. _buildMetrics no
    // longer uses byQtrAbbr — see its own docblock.
    ...
    const [
      ...
      this._buildTableFromKpiGroup('financials.pnl.quarterly', resCtx),
      this._buildTableFromKpiGroup('financials.pnl.annual', resCtx),
      this._buildTableFromKpiGroup('financials.balance-sheet.annual', resCtx),
      this._buildTableFromKpiGroup('financials.balance-sheet.quarterly', resCtx),
      this._buildTableFromKpiGroup('financials.cashflow.annual', resCtx),
      this._buildTableFromKpiGroup('financials.cashflow.quarterly', resCtx),
      this._buildTTM(byQtrAbbr, mktCapCr),
      this._buildMetrics(annualPL, resCtx),
  ...
  // Which ScreenConfig actually applies for this company: a company-group-
  // scoped variant (ScreenConfig.variant_of_key === configKey, e.g. a
  // BFSI-only P&L with its own KpiGroup tree) if the company is a member of
  // that variant's company_group_slug, else the base config itself. This is
  // the explicit, admin-authored replacement for the old silent
  // KpiRelationship 'variant_for_group' formula swap — a BFSI company gets a
  // whole separate, visible set of rows, not a hidden per-abbr substitution.
  // Multiple variants for the same configKey are allowed (e.g. future
  // industry-specific tables); the first whose group the company belongs to
  // wins, so admin ordering of variant creation matters if groups ever
  // overlap (same accepted convention as
  // services/companyGroups/resolver.js#resolveConfigKeyForTicker).
  async _resolveScreenConfig(configKey, resCtx) {
    const variants = await prisma.screenConfig.findMany({ where: { variant_of_key: configKey } });
    for (const variant of variants) {
      if (variant.company_group_slug && await resCtx.isCompanyInGroup(variant.company_group_slug)) {
        return variant;
      }
    }
    return prisma.screenConfig.findUnique({ where: { key: configKey } });
  }
  ...
  async _buildTableFromKpiGroup(configKey, resCtx) {
    const config = await this._resolveScreenConfig(configKey, resCtx);
  ...
  async _buildMetrics(annualPeriods, resCtx) {
    const [revValues, profitValues, roeValues, ttmRevSeries, ttmPatSeries, priceAnnualSeries] = await Promise.all([
    ...
    // cagrFromSeries silently uses whatever's available when a window can't
    // be fully filled (e.g. a company with 4Y of history "filling" a 10Y
    // request) rather than refusing — correct for REV_CAGR_3Y etc. because
    // windowedCagr below guards point-count first, but STOCK_CAGR_1Y/3Y/5Y/
    // 10Y have no such wrapper, so the same guard is applied here before
    // trusting each one: fewer than windowYears+1 real annual price points
    // means null, not a silently-shortened value mislabeled under a bigger
    // window.
    const pricePointCount = priceAnnualSeries.filter((p) => p.value != null).length;
    const stockCagrIfEnoughHistory = async (windowYears, metricId) => {
      if (pricePointCount < windowYears + 1) return null;
      const res = await resolveMetric(metricId, resCtx, { frequency: 'annual' });
      return r2(res.value);
    };
    const [stockCagr1y, stockCagr3y, stockCagr5y, stockCagr10y] = await Promise.all([
      stockCagrIfEnoughHistory(1, 'STOCK_CAGR_1Y'),
      stockCagrIfEnoughHistory(3, 'STOCK_CAGR_3Y'),
      stockCagrIfEnoughHistory(5, 'STOCK_CAGR_5Y'),
      stockCagrIfEnoughHistory(10, 'STOCK_CAGR_10Y'),
    ]);
