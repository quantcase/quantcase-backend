...
class Financials {
  ...
  async analyze(symbol) {
    ...
    const [byAnnualAbbr, byQtrAbbr, marketSnap, peRows, hist10yRows, hist5yRows] = await Promise.all([
      ...
      fetchMonthlyClose(prisma, symbol, { since: fiveYrsAgo }),
    ]);
  ...
  _buildMetrics(annualPeriods, hist10y, hist5y) {
    ...
    const salesGrowth3y  = n >= 4  ? roundCagr(revSeries[n - 4],  revSeries[n - 1],  3) : null;
    const salesGrowth5y  = n >= 6  ? roundCagr(revSeries[n - 6],  revSeries[n - 1],  5) : null;
    const salesGrowth10y = n >= 11 ? roundCagr(revSeries[n - 11], revSeries[n - 1], 10) : null;

    const profitGrowth3y  = n >= 4  ? roundCagr(profitSeries[n - 4],  profitSeries[n - 1],  3) : null;
    const profitGrowth10y = n >= 11 ? roundCagr(profitSeries[n - 11], profitSeries[n - 1], 10) : null;
    ...
    const stockCagr10y = roundCagr(firstClose(hist10y), lastClose(hist10y), 10);
    const stockCagr5y  = roundCagr(firstClose(hist5y),  lastClose(hist5y),  5);
