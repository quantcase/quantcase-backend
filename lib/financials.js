...
const { fetchMarketSnapshot, getRollingStockPriceCagr } = require('../utils/formulaRegistry/dataFetcherMarket');
const { resolveScreenConfig } = require('./screenConfigResolver');
...
class Financials {
  ...
  async analyze(symbol) {
    ...
    const resCtxC = createResolutionContext({ symbol, reportType: 'C' });
    const resCtxS = createResolutionContext({ symbol, reportType: 'S' });

    const [byAnnualAbbr, byQtrAbbr, marketSnap] = await Promise.all([
    ...
    const [
      pnlQuarterlyC, pnlQuarterlyS,
      pnlAnnualC, pnlAnnualS,
      balanceSheetAnnualC, balanceSheetAnnualS,
      balanceSheetQuarterlyC, balanceSheetQuarterlyS,
      cashFlowAnnualC, cashFlowAnnualS,
      cashFlowQuarterlyC, cashFlowQuarterlyS,
      ttm, metricsC, metricsS, valuationC, valuationS,
    ] = await Promise.all([
      this._buildTableFromKpiGroup('financials.pnl.quarterly', resCtxC),
      this._buildTableFromKpiGroup('financials.pnl.quarterly', resCtxS),
      this._buildTableFromKpiGroup('financials.pnl.annual', resCtxC),
      this._buildTableFromKpiGroup('financials.pnl.annual', resCtxS),
      this._buildTableFromKpiGroup('financials.balance-sheet.annual', resCtxC),
      this._buildTableFromKpiGroup('financials.balance-sheet.annual', resCtxS),
      this._buildTableFromKpiGroup('financials.balance-sheet.quarterly', resCtxC),
      this._buildTableFromKpiGroup('financials.balance-sheet.quarterly', resCtxS),
      this._buildTableFromKpiGroup('financials.cashflow.annual', resCtxC),
      this._buildTableFromKpiGroup('financials.cashflow.annual', resCtxS),
      this._buildTableFromKpiGroup('financials.cashflow.quarterly', resCtxC),
      this._buildTableFromKpiGroup('financials.cashflow.quarterly', resCtxS),
      this._buildTTM(byQtrAbbr, mktCapCr),
      this._buildMetrics(resCtxC),
      this._buildMetrics(resCtxS),
      this._buildValuation(mktCapCr, resCtxC),
      this._buildValuation(mktCapCr, resCtxS),
    ]);
    ...
    return {
      ...
      standardized: {
        quarterly: { C: pnlQuarterlyC, S: pnlQuarterlyS },
        annual: { C: pnlAnnualC, S: pnlAnnualS },
        balanceSheet: { 
          annual: { C: balanceSheetAnnualC, S: balanceSheetAnnualS }, 
          quarterly: { C: balanceSheetQuarterlyC, S: balanceSheetQuarterlyS } 
        },
        cashFlow: { C: cashFlowAnnualC, S: cashFlowAnnualS },
        cashFlowQuarterly: { C: cashFlowQuarterlyC, S: cashFlowQuarterlyS },
        ttm,
        metrics: { C: metricsC, S: metricsS },
        valuation: { C: valuationC, S: valuationS },
      },
    };
  }
  ...
  async _buildTableFromKpiGroup(configKey, resCtx) {
    ...
    // Cap oldest period shown to FY2020 (Q1 FY2020 for quarterly, FY20 for annual)
    let startIndex = rawPeriods.findIndex((p) => {
      const yr = parseInt(p.fiscal_year?.replace(/\D/g, ''), 10);
    ...
    let periodEntries = rawPeriods.slice(startIndex).map((p) => ({ fiscal_year: p.fiscal_year, quarter: p.quarter }));
    if (config.periods_shown && periodEntries.length > config.periods_shown) {
      const extra = periodEntries.length - config.periods_shown;
      startIndex += extra;
      periodEntries = periodEntries.slice(extra);
    }
    ...
    const rows = (await Promise.all(
      group.children.map((node) => this._buildGroupRow(node, resCtx, freq, sliceFrom, config.decimal_places, periodEntries.length))
    )).filter(Boolean);

    return { periods: labels, rows };
  }

  async _buildGroupRow(node, resCtx, freq, sliceFrom, decimals, totalPeriods) {
    if (node.company_group_slug && !(await resCtx.isCompanyInGroup(node.company_group_slug))) return null;

    const def = node.kpi_abbr ? await getDefinition(node.kpi_abbr) : null;
    const rawValues = node.kpi_abbr
      ? await resolveFormulaSeries(node.kpi_abbr, resCtx, { frequency: freq })
      : [];
    let values = rawValues?.length
      ? rawValues.slice(sliceFrom, sliceFrom + totalPeriods).map((v) => roundTo(v, decimals))
      : [];
    if (values.length < (totalPeriods ?? 0)) {
      values = values.concat(new Array((totalPeriods ?? 0) - values.length).fill(null));
    }

    const row = { key: node.kpi_abbr || node.slug, label: node.label, values };
    if (def?.denomination === 'percentage') row.format = 'percent';

    if (node.children?.length) {
      const childRows = (await Promise.all(
        node.children.map((c) => this._buildGroupRow(c, resCtx, freq, sliceFrom, decimals, totalPeriods))
      )).filter(Boolean);
      if (childRows.length) {
  ...
  // Fully registry-driven now: revSeries/profitSeries/roeSeries all come from
  // resolveFormulaSeries against resCtx (fallback_abbrs-aware, e.g. REV_OP's
  // fallback to TOTAL_INCOME is applied by the resolver itself, not a
  // hand-rolled `p['REV_OP'] ?? p['TOTAL_INCOME']` duplicate of that same
  // rule). TTM growth reads real per-quarter TTM_REV_OP/TTM_PAT series (their
  // SUM(...,4) formulas now resolve correctly at every historical index, see
  // financial.js#_resolveAtIndex) instead of a separate hand-rolled quarterly
  // sum. Stock Price CAGR is plain STOCK_CAGR_1Y/3Y/5Y/10Y Kpi formulas
  // (CAGR(PRICE, N)) — PRICE resamples to annual via its DAILY_RESAMPLE_MODE
  // policy (dataFetcherMarket.js), the same mechanism that makes any
  // daily-native abbr usable at a coarser frequency. A young/recently-listed
  // company simply won't have enough history for the longer windows — null
  // is the correct, expected answer there, not an error to explain.
  async _buildMetrics(resCtx) {
    const [seriesMap, revValues, profitValues, roeValues, ttmRevSeries, ttmPatSeries, { '1y': stockCagr1y, '3y': stockCagr3y, '5y': stockCagr5y, '10y': stockCagr10y }] = await Promise.all([
      resCtx.getSeriesMap('annual'),
      resolveFormulaSeries('REV_OP', resCtx, { frequency: 'annual' }),
      resolveFormulaSeries('PAT', resCtx, { frequency: 'annual' }),
      resolveFormulaSeries('ROE', resCtx, { frequency: 'annual' }),
      resolveFormulaSeries('TTM_REV_OP', resCtx, { frequency: 'quarterly' }),
      resolveFormulaSeries('TTM_PAT', resCtx, { frequency: 'quarterly' }),
      // resCtx.getSeries('PRICE', 'annual'),
      getRollingStockPriceCagr(prisma, resCtx.symbol),
    ]);

    const anyAbbr = Object.keys(seriesMap)[0];
    const annualPeriods = anyAbbr ? seriesMap[anyAbbr] : [];
    ...
    const revSeries = annualPeriods.map((p, i) => ({ value: revValues[i] ?? null, fiscal_year: p.fiscal_year }));
    const profitSeries = annualPeriods.map((p, i) => ({ value: profitValues[i] ?? null, fiscal_year: p.fiscal_year }));
    const roeSeries = annualPeriods.map((p, i) => ({ value: roeValues[i] ?? null, fiscal_year: p.fiscal_year }));

    // TTM growth: current trailing-4-quarter figure vs the same figure 4
    // quarters ago (a YoY comparison of two TTM snapshots).
    const revTtmCurr = ttmRevSeries.at(-1) ?? null;
    const revTtmPrior = ttmRevSeries.length > 4 ? ttmRevSeries.at(-5) : null;
    const patTtmCurr = ttmPatSeries.at(-1) ?? null;
    const patTtmPrior = ttmPatSeries.length > 4 ? ttmPatSeries.at(-5) : null;
    const quartersAvail = ttmRevSeries.filter((v) => v != null).length;
  ...
  async _buildValuation(mktCapCr, resCtx) {
    ...
    //
    // Each ratio below gets its own explicit frequency — NOT one cadence for
    // the whole section — because each is structurally tied to a specific
    // Prowess/nse_equity_new dataset, not an admin preference:
    //   - PE_DAILY: 'daily' (PRICE/EPS_DAILY, both nse_equity_new-native).
    //   - PB_TTM/ENTERPRISE_VALUE: 'annual' — their balance-sheet terms
    //     (NET_WORTH/BORR_TOTAL/CASH_EQUIV) are only reliably populated
    //     annually; NET_WORTH specifically has ZERO quarterly rows anywhere.
    //   - EV_EBITDA/EV_REVENUE: 'quarterly' — their TTM_EBITDA/TTM_REV_OP
    //     terms are quarterly-pinned-by-definition (trailing 4 quarters).
    //     (Currently null for every company regardless — PBT/TAX_EXP have
    //     zero quarterly rows anywhere, a separate pre-existing Prowess
    //     quarterly-ingestion gap, not a frequency problem.)
    //   - DIVIDEND_YIELD: 'annual' (dividends are an annual concept; also
    //     currently has zero source data either way, see below).
    const [peRes, pbRes, evRes, evEbitdaRes, evRevenueRes, divYieldRes, nwSeries, epsSeries] = await Promise.all([
      resolveMetric('PE_DAILY', resCtx, { frequency: 'daily' }),
      resolveMetric('PB_TTM', resCtx, { frequency: 'annual' }),
      resolveMetric('ENTERPRISE_VALUE', resCtx, { frequency: 'annual' }),
      resolveMetric('EV_EBITDA', resCtx, { frequency: 'quarterly' }),
      resolveMetric('EV_REVENUE', resCtx, { frequency: 'quarterly' }),
      ...
      resolveMetric('DIVIDEND_YIELD', resCtx, { frequency: 'annual' }),
