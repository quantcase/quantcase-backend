...
class Financials {
  ...
  async analyze(symbol) {
    // ── 1. Resolve company name from osc_identity via earnings_calls ───────────
    const ecRow = await prisma.earnings_calls.findFirst({
      where:  { company: symbol },
      select: { company_name: true },
    });
    // Prowess uses "Ltd." while earnings_calls stores "Limited" / "LIMITED"
    const rawName = ecRow?.company_name ?? null;
    const companyName = rawName
      ? rawName.replace(/\bLIMITED\b/g, 'Ltd.').replace(/\bLimited\b/g, 'Ltd.')
      : null;

    // ── 2. Fetch all data in parallel ─────────────────────────────────────────
    const now        = Date.now();
    const twoYrsAgo  = new Date(now - 2  * 365 * 24 * 60 * 60 * 1000);
    const tenYrsAgo  = new Date(now - 10 * 365 * 24 * 60 * 60 * 1000);
    const fiveYrsAgo = new Date(now -  5 * 365 * 24 * 60 * 60 * 1000);

    const [prowessRows, mktCapRows, peRows, hist10yRows, hist5yRows] = await Promise.all([
      // All KPI values for this company (annual + quarterly)
      companyName
        ? prisma.$queryRaw`
      ...
      // Latest market cap
      prisma.$queryRaw`
        SELECT "market_cap(Cr)" AS market_cap_cr, date
        FROM market_cap
        WHERE symbol = ${symbol}
        ORDER BY date DESC
        LIMIT 1
    ...
    // ── 3. Organise prowess rows ──────────────────────────────────────────────
    // All periods sorted asc (already from DB ORDER BY)
    const byAbbr = {};   // abbr → [{fiscal_year, quarter, value, raw_value, multiplier}]
    for (const row of prowessRows) {
      if (!byAbbr[row.kpi_abbr]) byAbbr[row.kpi_abbr] = [];
      byAbbr[row.kpi_abbr].push(row);
    }

    // Helpers to get series in Cr for a given abbr
    const seriesCr = (abbr) => (byAbbr[abbr] ?? []).map((r) => this._toCr(r));
    const latestCr = (abbr) => {
    ...
    // Annual P&L: Q4 entries (full-year figures)
    const annualPL    = this._filterAnnual(byAbbr);
    const quarterlyPL = this._filterQuarterly(byAbbr);
    ...
    return {
      ...
      standardized: {
        ...
        ttm:               this._buildTTM(byAbbr, mktCapCr),
        metrics:           this._buildMetrics(annualPL, hist10yRows, hist5yRows),
        valuation:         this._buildValuation(byAbbr, mktCapCr, peRows),
      },
    };
  }
  ...
  _toCr(row) {
    if (!row) return null;
    // raw_value is already in Cr; value is raw_value * multiplier
    if (row.raw_value != null && row.raw_value !== '') {
      const v = parseFloat(row.raw_value);
      return isNaN(v) ? null : r2(v);
    }
    if (row.value != null && row.multiplier) {
      return r2(parseFloat(row.value) / row.multiplier);
    }
    if (row.value != null) return r2(parseFloat(row.value));
    return null;
  }
    ...
    const build = (periods) => {
      ...
      return {
        ...
        rows: [
          ...
          { key: 'totalLiabilities', label: 'Total Liabilities', values: pick('TOTAL_LIAB'), highlight: true },
          ...
          { key: 'totalAssets',      label: 'Total Assets',      values: pick('TOTAL_ASSETS'), highlight: true },
        ],
      };
    };
