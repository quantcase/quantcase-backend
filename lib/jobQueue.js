...
class JobQueue {
  ...
  async addJob(queueName, jobData, options = {}) {
    ...
    try {
      ...
      // Generate a UUID upfront — used as both BullMQ job ID and DB bullmqId
      // This avoids BullMQ's auto-increment IDs which reset on Redis restart
      // and cause unique constraint violations in the database
      ...
      bullmqJob = await queue.add(
        ...
        jobData,
        {
          ...options,   // spread first so explicit jobId below always wins
          jobId,
      ...
      // Create database entry with the same UUID as BullMQ job ID
      ...
      // If database creation failed but BullMQ job was created, remove it
      // to avoid orphaned jobs sitting in the queue with no DB record
      if (bullmqJob) {
        try {
