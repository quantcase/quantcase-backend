...
class JobQueue {
  ...
  /**
   * Add a chained flow (parent waits for all children to complete)
   * @param {object} tree - FlowProducer job tree
   * @returns {Promise<object>} Root job of the flow
   */
  async addFlow(tree) {
    const flow = await this.flowProducer.add(tree);
    console.log(`BullMQ flow created — parent: ${flow.job.id} in queue: ${tree.queueName}`);
    return flow;
  }
