...
function buildTabSummaries(tags, anchors) {
  const s = anchors?.support != null ? `around Rs.${formatPrice(anchors.support)}` : 'support';
  const r = anchors?.resistance != null ? `Rs.${formatPrice(anchors.resistance)}` : 'resistance';
  const lta = anchors?.sma200 != null ? `Rs.${formatPrice(anchors.sma200)}` : 'long term average';
  const mta = anchors?.sma100 != null ? `Rs.${formatPrice(anchors.sma100)}` : 'medium term average';
...
function computePriorityWatchout(inputs, lowestModuleId, anchors) {
  ...
  const sup = anchors?.support != null ? `Rs.${formatPrice(anchors.support)}` : 'support';
...
function buildActionableInsights(tradeLevels, finalScore, anchors) {
  const res = anchors?.resistance != null ? `Rs.${formatPrice(anchors.resistance)}` : 'resistance';

  const formatHorizon = (h, levels) => {
    const entry = levels?.idealEntry != null ? `Rs.${formatPrice(levels.idealEntry)}` : res;
    const stop = levels?.stopLoss != null ? `Rs.${formatPrice(levels.stopLoss)}` : 'support';
