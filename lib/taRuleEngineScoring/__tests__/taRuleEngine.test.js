...
describe('Trade Levels & Buffers', () => {
  ...
  test('roundPrice and formatPrice retain 2 decimal places for values < 100 and round to integer for values >= 100', () => {
    // Values >= 100 -> integer rounding
    assert.equal(roundPrice(1234.56), 1235);
    assert.equal(roundPrice(100.4), 100);
    assert.equal(formatPrice(1234.56), '1235');
    assert.equal(formatPrice(5800.2), '5800');

    // Values < 100 -> up to 2 decimal places
    assert.equal(roundPrice(45.254), 45.25);
    assert.equal(roundPrice(45.256), 45.26);
    assert.equal(roundPrice(8.5), 8.5);
    assert.equal(roundPrice(0.758), 0.76);
    assert.equal(formatPrice(45.254), '45.25');
    assert.equal(formatPrice(8.5), '8.5');
    assert.equal(formatPrice(null), '-');
  });

  test('Preserves 2 decimal places in computeTradeLevels for stocks < 100', () => {
    const levels = computeTradeLevels(
      {
        sr_support_price: 42.50,
        sr_resistance_price: 48.75,
        sma20: 46.20,
        sma50: 44.10,
        sma100: 41.50,
        sma200: 38.00,
    ...
    // idealEntry should preserve decimals
    assert.equal(levels.swing.idealEntry, 48.75);
    assert.equal(levels.priceAnchors.support, 42.5);
    assert.equal(levels.priceAnchors.resistance, 48.75);

    // stopLoss and target must not be prematurely rounded to integers
    assert.ok(levels.swing.stopLoss < 48.75);
    assert.equal(levels.swing.stopLoss, Math.round(levels.swing.stopLoss * 100) / 100);
    assert.ok(levels.swing.target > 48.75);
    assert.equal(levels.swing.target, Math.round(levels.swing.target * 100) / 100);
  });

  test('Preserves 2 decimal places in computeLevelsToWatch for stocks < 100', () => {
    const res = computeLevelsToWatch(
      {
        sr_support_price: 42.35,
        s1: 40.15,
        sma200: 38.45,
      },
      'Swing Entry'
    ...
    assert.equal(res.levelsArray[0].price, 42.35);
    assert.equal(res.levelsArray[1].price, 42.35);
    assert.equal(res.levelsArray[2].price, 38.45);
    assert.ok(res.horizonNote.includes('Rs.42.35'));
  });
});
