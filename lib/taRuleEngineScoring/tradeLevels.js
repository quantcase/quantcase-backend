...
function computeTradeLevels(inputs, playbookResult) {
  ...
  const anchors = { support, resistance, sma20, sma50, sma100, sma200 };
  ...
  function evaluateStop(idealEntry, minPct, maxPct, clampPct) {
    ...
    if (qualifying.length === 1) {
      return Math.round(qualifying[0].stop);
    }
    ...
    if (qualifying.length > 1) {
      ...
      if (Math.abs(qualifying[0].distPct - qualifying[1].distPct) <= 1) {
        ...
        if (supMatch) return Math.round(supMatch.stop);
      }
      return Math.round(qualifying[0].stop);
    }

    // Clamp if none qualify
    return Math.round(idealEntry * (1 - clampPct / 100));
  }
  ...
  if (swingEntry != null && swingStop != null) {
    ...
    swingTarget = Math.round(swingTarget);
  }
  ...
  if (invEntry != null && invStop != null) {
    ...
    invTarget = Math.round(invTarget);
  }

  return {
    priceAnchors: anchors,
    swing: {
      idealEntry: swingEntry != null ? Math.round(swingEntry) : null,
      stopLoss: swingStop,
      target: swingTarget,
    },
    positional: {
      idealEntry: posEntry != null ? Math.round(posEntry) : null,
      stopLoss: posStop,
      target: posTarget,
    },
    investor: {
      idealEntry: invEntry != null ? Math.round(invEntry) : null,
      stopLoss: invStop,
      target: invTarget,
    },
  };
...
function computeLevelsToWatch(inputs, idealFor) {
  const immediatePrice = inputs.sr_support_price != null ? parseFloat(inputs.sr_support_price) : null;
  const structuralPrice = inputs.sr_support_price != null ? parseFloat(inputs.sr_support_price) : (inputs.s1 != null ? parseFloat(inputs.s1) : null);
  const regimePrice = inputs.sma200 != null ? parseFloat(inputs.sma200) : null;
