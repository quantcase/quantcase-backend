 ...
 *
 * This does NOT adjust prices — back-adjusting would silently rewrite a genuine crash.
 * It only reports what it saw so callers can distrust the result. The real fix is
 * ingesting adjusted prices.
 *
...
function classifyRange(priorDir, s) {
  if (priorDir === 'down') {
    ...
    if (s.vd) score += 1;
    // Without a climax or a spring, a range near the lows isn't accumulation.
 ...
 */
function buildWyckoffResponse({ symbol, bars, result, era, totalRows, options = {} }) {
  const { chartYears = 3, includeBars = true } = options;
  const n = bars.length;
  const asOf = n ? bars[n - 1].date : null;
  const r = result;
  const splits = detectSuspectedSplits(bars);
  ...
  const response = {
    ...
    meta: {
      ...
      // Prices are NOT split-adjusted upstream. When this is true the phase call is
      // unreliable — a split reads as a crash. Surface it in the UI, don't hide it.
