...
// NSE symbol → NSE Basic Industry classification, from the same osc_identity.csv.
// Kept separate from loadIdentityMap (which callers rely on returning name
// strings) so it can be looked up when creating earnings_calls rows without
// a follow-up backfillCompanyMeta run.
function loadIndustryMap() {
  if (_industryMap) return _industryMap;
  const raw = fs.readFileSync(path.join(__dirname, 'osc_identity.csv'), 'utf-8');
  const content = raw.charCodeAt(0) === 0xfeff ? raw.slice(1) : raw;
  const rows = csvParse.parse(content, { columns: true, relax_column_count: true });
  _industryMap = {};
  for (const row of rows) {
    const sym = (row['NSE symbol'] || '').trim().toUpperCase();
    const industry = (row['NSE Basic Industry classification'] || '').trim();
    if (sym && industry) _industryMap[sym] = industry;
  }
  return _industryMap;
}
