...
// ─── Null-reason diagnosis ────────────────────────────────────────────────────
// Mirrors exactly the derive([...required], fn) calls in finDerivedKpis.js.
// Only lists inputs that are passed to derive() — inputs treated as ?? 0 are not included.
...
function nullReason(abbr, raw, derived, bfsi) {
  ...
  switch (abbr) {
    case 'EBIT':
      // derive([REV_OP, EMP_EXP, DEP_AMORT, OTH_EXP], ...)
      // COST_MAT / PURCH_STOCK / INV_CHG handled as ?? 0 — not required
    ...
    case 'EBIT_MARGIN':
      // derive([ebitVal, REV_OP], ([eb, rev]) => rev === 0 ? null : ...)
    ...
    case 'ROCE':
      ...
      if (r('TOTAL_ASSETS') != null && r('CURR_LIAB') != null) {
        if ((r('TOTAL_ASSETS') - r('CURR_LIAB')) === 0) {
          zeroDenom.push('TOTAL_ASSETS − CURR_LIAB = 0  (Capital Employed is zero)');
        }
      }
      break;

    case 'ROA':
      for (const k of ['PAT', 'TOTAL_ASSETS']) {
        if (r(k) == null) missing.push(k);
      }
    ...
    case 'ROE':
      for (const k of ['PAT', 'EQ_SHARE_CAP', 'RES_SURPLUS']) {
        if (r(k) == null) missing.push(k);
      }
    ...
    case 'CAPEX':
      // Only ASSET_PPE is required; ASSET_CWIP treated as ?? 0
...
/**
 * Build a flat metrics array from raw + derived Q4 batches.
 * Each entry has: abbr, label, type, formula?, value, period, status, null_reason?
 */
function buildMetrics({ rawAbbrs, rawBatch, derivedBatch, bfsi }) {
  ...
  for (const abbr of DERIVED_ORDER) {
    ...
    const formula = abbr === 'EBIT' ? (bfsi ? FORMULAE.PPOP : FORMULAE.EBIT)
                  ...
                  : FORMULAE[abbr] ?? null;
    const label   = abbr === 'EBIT' && bfsi ? 'PPOP (Pre-Provisioning Operating Profit)'
                  : KPI_LABELS[abbr] ?? abbr;
...
/** Wrap a CAGR/latest result object into a clean metric entry */
function cagrMetric({ abbr, label, source, result }) {
  return {
    abbr,
    label,
    type:    'computed',
    formula: FORMULAE.CAGR,
    source,
...
async function getOpportunityStats(req, res) {
  const { callId } = req.query;
  if (!callId) {
    return res.status(400).json({ success: false, error: 'callId query param is required' });
  }

  try {
    const call = await prisma.earnings_calls.findUnique({ where: { id: callId } });
    if (!call) {
      return res.status(404).json({ success: false, error: `Earnings call "${callId}" not found` });
    }
    ...
    // CUST fallback: transcript summary if QE data is missing
    ...
    const industryMetrics = buildMetrics({
      rawAbbrs:     INDUSTRY_RAW_ABBRS,
      rawBatch:     industryRawBatch,
      derivedBatch,
      bfsi,
    });
    ...
    const competitionMetrics = [
      ...
      industryEps
        ? cagrMetric({ abbr: 'industry_eps_cagr', label: 'Industry EPS CAGR (avg)', source: 'summaryNew.kpis → EPS_BASIC (all industry tickers)', result: industryEps })
        : null,
      industryPe
        ? cagrMetric({ abbr: 'industry_pe_cagr',  label: 'Industry P/E CAGR (avg)', source: 'pe_data table (all industry tickers)',               result: industryPe })
        : null,
    ].filter(Boolean);

    const fsMetrics = buildMetrics({
      rawAbbrs:     FINANCIAL_STRENGTH_RAW_ABBRS,
      rawBatch:     fsRawBatch,
      derivedBatch,
      bfsi,
    });
    ...
    const custMetrics = [
      {
        abbr:   'CUST',
        label:  'Number of Customers (latest)',
        type:   'raw',
        source: custSource,
        value:  resolvedCustLatest.value,
        period: resolvedCustLatest.period,
        status: resolvedCustLatest.value != null ? 'ok' : 'null',
      },
      ...
      sections: [
        {
          ...
          note:             'Q4 (annual) snapshots only — same data injected into LLM prompt',
          peers_discovered: peerTickers,
          metrics:          industryMetrics,
        },
        {
          id:      'competition',
          label:   'Competition',
          note:    'Computed CAGR values injected into LLM prompt',
          metrics: competitionMetrics,
        },
        {
          id:      'financial_strength',
          label:   'Financial Strength',
          note:    'Q4 (annual) snapshots — latest value shown; full 10Q timeseries also injected into prompt',
          metrics: fsMetrics,
        },
        {
          id:      'customer_traction',
          label:   'Customer Traction',
          note:    'Latest customer count + CAGR injected into LLM prompt',
          metrics: custMetrics,
        },
      ],
    });
    ...
    console.error('[adminController] getOpportunityStats error:', err);
