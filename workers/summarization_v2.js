...
worker.on('failed', async (job, err) => {
  wlog.error(`[summarization-v2] Job ${job.id} failed (attempt ${job.attemptsMade}): ${err.message}`);
  ...
  await prisma.pipelineJobFailure.upsert({
    ...
    create: { queue: 'summarization_v2', bullmq_job_id: String(job.id), call_id: d.callId ?? '', source_doc_type: 'transcript', chunk_index: d.chunkIndex ?? null, total_chunks: d.totalChunks ?? null, lineage_id: d.lineageId ?? null, error_message: err.message, attempts_made: job.attemptsMade },
    update: { error_message: err.message, attempts_made: job.attemptsMade },
  }).catch(e => wlog.error(`[summarization-v2] Failed to record failure: ${e.message}`));
});
